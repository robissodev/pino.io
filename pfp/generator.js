...
avatarImage.onerror = function () {
    ...
    avatarImage.src = '/pfp-images/default/fallback_image.png';
};

// Accessories
const folder = '/pfp-images/';
const accessories = {
...
function setDefaultAvatar() {
    avatarImage.src = folder + "default/default_pino.png";

    // Clear existing layers except background and watermark
    clearLayersExceptBackgroundAndWatermark();

    // Add default layers
    addOrReplaceCategoryImage('background-layer', folder + "default/default_background.png", -3);
}
...
function ensureWatermarkLayer() {
    ...
    if (!watermarkLayer) {
        ...
        watermarkLayer.src = folder + 'default/watermark.png';
...
document.querySelectorAll('.cores-option').forEach(option => {
    option.addEventListener('click', () => {
        ...
        if (img) {
            ...
            addOrReplaceCategoryImage('cores-layer', `${folder}cores/${img}`, layerZIndex.cores);
        }
    });
});
...
document.querySelectorAll('.based-option').forEach(option => {
    option.addEventListener('click', () => {
        ...
        if (img) {
            ...
            addOrReplaceCategoryImage('based-layer', `${folder}based/${img}`, layerZIndex.based);
        }
    });
});
...
Object.keys(accessories).forEach(category => {
    ...
    options.forEach(option => {
        option.addEventListener('click', () => {
            const img = option.getAttribute('data-img');
            if (img) {
                addOrReplaceCategoryImage(`${category}-layer`, `${folder}${category}/${img}`);
            }
        });
    });
});
...
generateBtn.addEventListener('click', () => {
    ...
    Object.keys(accessories).forEach(category => {
        ...
        if (Math.random() <= probability && items.length > 0) {
            ...
            addOrReplaceCategoryImage(`${category}-layer`, `${folder}${category}/${randomItem}`);
        } else {
